...
<section class="preview-panel" class:engraving-preview-panel={studio.project.outputMode === "engraving"}>
  <div class="preview-toolbar"><div class="ldt-toggle-group mode-switch" role="radiogroup" aria-label="Preview mode">{#each studio.previewModeOptions as option}<button type="button" class="ldt-toggle-group__item" role="radio" aria-checked={studio.mode === option.value} data-state={studio.mode === option.value ? "on" : "off"} tabindex={keyboardView === option.value ? 0 : -1} onclick={() => { if (option.value === "2d" && studio.selectedLayer === 0) studio.selectedLayer = featuredLayerIndex(studio.geometry); studio.previewNotice = ""; if (option.value === "3d") studio.threeUnavailable = false; studio.mode = option.value as PreviewMode; }} onkeydown={navigateChoice}>{#if option.value === "map"}<MapIcon size={15} />{:else if option.value === "engraving"}<PenTool size={15} />{:else if option.value === "2d"}<Layers3 size={15} />{:else if option.value === "custom"}<Waves size={15} />{:else if option.value === "export"}<FileOutput size={15} />{:else}<Box size={15} />{/if}{option.label}</button>{/each}</div><div class="preview-readout"><span>{shownLength(studio.project.widthMm)} × {shownLength(studio.project.heightMm)} {studio.shownLengthUnit}</span>{#if !studio.embeddedInPlatform}<span class="output-summary">{#each studio.outputSummary as item}<span>{item}</span>{/each}</span>{/if}<span>{Math.round(displayElevation(studio.geometry.minElevationM, studio.project.units)).toLocaleString()}–{Math.round(displayElevation(studio.geometry.maxElevationM, studio.project.units)).toLocaleString()} {studio.shownElevationUnit}</span></div></div>
  <div class="preview-stage" aria-busy={studio.previewBusy} data-placement-graphics={studio.project.customGraphics?.length ? "" : undefined} data-placement-fade={studio.placementFade || undefined} data-road-markings={studio.detailCounts.road} data-trail-markings={studio.detailCounts.trail} data-transportation-label-markings={studio.detailCounts.transportationLabel} data-water-markings={studio.detailCounts.water} data-contour-markings={studio.detailCounts.contour} data-alignment-markings={studio.detailCounts.alignment} data-elevation-markings={studio.detailCounts.elevation} data-north-markings={studio.detailCounts.north} data-scale-markings={studio.detailCounts.scale} data-plaque-markings={studio.detailCounts.plaque} data-marker-markings={studio.detailCounts.marker} data-custom-line-markings={studio.detailCounts.customLine}>{#if !studio.embeddedInPlatform}<FeedbackButton edge getContext={getFeedbackContext} />{/if}{#if studio.mode === "map" && studio.placementBackdrop !== "3d"}<MapStage onUnavailable={(reason) => { studio.mode = studio.project.outputMode === "engraving" ? "engraving" : "2d"; studio.previewNotice = reason === "load-failed" ? "Map could not load · check your connection or choose a location using search or coordinates" : "Map is unavailable in this browser · choose a location using search or coordinates"; }} />{:else if studio.mode === "engraving" && studio.placementBackdrop !== "3d"}{#if studio.EngravingPreview}<EngravingPreview geometry={studio.geometry} project={studio.project} cropShape={studio.sourceProject.cropShape} />{:else if studio.engravingPreview.failed}<div class="preview-loading preview-load-failed" role="alert">Engraving preview could not load<button type="button" class="btn btn-secondary" onclick={() => studio.engravingPreview.load()}>Retry</button></div>{:else}<div class="preview-loading">Loading engraving…</div>{/if}{:else if studio.mode === "custom" && studio.placementBackdrop !== "3d"}{#if CustomDataView}<CustomDataView />{:else if studio.customDataView.failed}<div class="preview-loading preview-load-failed" role="alert">Custom data could not load<button type="button" class="btn btn-secondary" onclick={() => studio.customDataView.load()}>Retry</button></div>{:else}<div class="preview-loading">Loading custom data…</div>{/if}{:else if studio.mode === "export" && studio.placementBackdrop !== "3d"}{#if ExportPreview}<ExportPreview geometry={studio.geometry} project={studio.project} busy={studio.previewBusy} />{:else if studio.exportPreview.failed}<div class="preview-loading preview-load-failed" role="alert">Export preview could not load<button type="button" class="btn btn-secondary" onclick={() => studio.exportPreview.load()}>Retry</button></div>{:else}<div class="preview-loading">Loading export preview…</div>{/if}{:else if studio.mode === "2d" && studio.placementBackdrop !== "3d"}{#if studio.TwoDPreview}<TwoDPreview geometry={studio.geometry} selectedLayer={studio.selectedLayer} />{:else if studio.twoDPreview.failed}<div class="preview-loading preview-load-failed" role="alert">Cut preview could not load<button type="button" class="btn btn-secondary" onclick={() => studio.twoDPreview.load()}>Retry</button></div>{:else}<div class="preview-loading">Loading cut preview…</div>{/if}{:else if studio.ThreePreview}<ThreePreview geometry={studio.geometry} exploded={studio.explodedPreview} placement={studio.placementBackdrop === "3d" ? { hiddenPrefixes: studio.placementPhase === "closing" ? [] : studio.placementHiddenPrefixes, marginMm: studio.placementMargin, hideMarkings: studio.placementPhase !== "closing", toolbarRows: studio.project.customGraphics?.length ? 2 : 1 } : undefined} onUnavailable={() => { studio.threeUnavailable = true; studio.mode = "2d"; studio.previewNotice = "3D is unavailable in this browser · showing cut layers"; }} />{:else}<div class="preview-loading">Loading 3D preview…</div>{/if}{#if studio.placementBackdrop && PlacementStage && (studio.placementBackdrop === "flat" || studio.ThreePreview)}<PlacementStage backdrop={studio.placementBackdrop} phase={studio.placementPhase} geometry={studio.geometry} project={studio.project} cropShape={studio.sourceProject.cropShape} session={studio.placement!} marginMm={studio.placementMargin} hiddenPrefixes={studio.placementHiddenPrefixes} onChange={(session) => { studio.placement = session; }} onDone={commitPlacement} onCancel={cancelPlacement} />{/if}{#if studio.mode === "3d" || studio.mode === "engraving"}<PlaceGraphicsButton />{/if}{#if studio.mode !== "map" && studio.mode !== "custom"}<div class="preview-attribution">Map data © <a href={OSM_ATTRIBUTION.url} target="_blank" rel="noreferrer">{OSM_ATTRIBUTION.name}</a> · <a href={`${base}/attribution${import.meta.env.VITE_SITE_ENV === "atomm" ? ".html" : ""}`} target="_blank" rel="noopener noreferrer">All sources<span class="ldt-visually-hidden"> (opens in a new tab)</span></a></div>{/if}{#if studio.previewBusy && studio.mode !== "custom"}<GenerationProgress compact={studio.detailsUpdating && studio.generationState !== "loading" && (!studio.terrainRefreshing || studio.mode === "map")} passive={studio.detailsUpdating && studio.generationState !== "loading"} title={studio.previewBusyLabel} detail={studio.status} step={studio.generationState === "loading" ? studio.generationStep : undefined} onCancel={studio.embeddedInPlatform && (studio.generationState === "loading" || (studio.terrainRefreshing && studio.mode !== "map")) ? cancelGeneration : undefined} cancelLabel={studio.generationState === "loading" ? "Cancel generation" : "Cancel"} />{/if}{#if studio.mode !== "custom" && studio.mode !== "export" && (studio.visibleWarnings.length || studio.previewNotice || studio.lakeDepthFittingOn)}
