...
<script lang="ts">
  ...
  const TIPS: Array<{ id: TipId; image: string; title: string; body: string[] }> = [
    { id: "place", image: place, title: "Pick a place", body: ["Choose a location, a suggested place, or drag the selection on the Map view. The terrain reloads for the new area on its own, so there is no Generate step to remember."] },
