<script lang="ts">
  import type { FeedbackContext, FeedbackType } from '$lib/site/feedback';
  ...
  let Dialog = $state.raw<typeof import('$lib/site/FeedbackDialog.svelte').default>();
  ...
  async function show() {
    ...
    try {
      Dialog ??= (await import('$lib/site/FeedbackDialog.svelte')).default;
      context = getContext?.();
      open = true;
    } catch { failed = true; }
