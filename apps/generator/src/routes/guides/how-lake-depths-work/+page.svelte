...
<Article title="How lake depths work" intro="A lake outline tells us where the water is, but not what lies beneath it. TopoStack combines published surveys, lake information, and surrounding terrain to build a lake floor for your map.">
  ...
  <section id="layers" aria-labelledby="layers-title">
    ...
    <dl class="settings">
      ...
      <div><dt>Material and sheet limits</dt><dd>A shallow lake may fall between contour levels and show no underwater step. Mountains can make each interval large. We reserve sheets for water within the 24-sheet limit: normally up to 6, more at higher depth exaggeration. Those sheets come from the land, so raising depth exaggeration can lower the applied land exaggeration. The result still has a finite vertical resolution.</dd></div>
      <div><dt>Fit lake depth to available layers</dt><dd>Off by default. When on, it compresses an over-deep lake to fit the available stack, keeping its waterline fixed and scaling depths uniformly; the studio shows the percentage of requested depth applied to each lake. Sea floors are not compressed. With fitting off, the deepest floor can be flattened, and the warning offers <strong>Fit depth</strong> to turn fitting on. <strong>Use manual depth</strong> turns it off again.</dd></div>
    </dl>
