...
<Article title="How lake depths work" intro="A lake outline tells us where the water is, but not what lies beneath it. TopoStack combines published surveys, lake information, and surrounding terrain to build a lake floor for your map.">
    ...
    <details>
      ...
      <ul class="provider-list">
        {#each data.sources as source (source.id)}
          <li><a href={source.url}>{source.name}</a><span>{source.region} · {source.kind === "grid" ? "Published floor grids" : "Survey contours"}</span></li>
        {/each}
      </ul>
    </details>
  ...
  <section id="surveys" aria-labelledby="surveys-title">
    ...
    <ol class="process">
      <li><strong>Prepare the published data.</strong> We convert supported source formats and units into map tiles. Some surveys report depth below a reference waterline; others report the floor’s elevation. We account for that difference using the dataset’s reference information.</li>
      ...
      <li><strong>Keep track of gaps.</strong> Available survey values take precedence. Other providers may fill uncovered cells. Remaining gaps use existing underwater terrain or a modeled basin when enough information exists; otherwise they remain at the waterline.</li>
    </ol>
    ...
    <p>Survey dates, detail, and reference water levels vary. For example, NOAA’s Lake Superior grid is a draft, and Swiss floor elevations use an approximate alignment to the lake surface. These datasets do not represent today’s water level.</p>
  </section>
  ...
  <section id="layers" aria-labelledby="layers-title">
    ...
    <dl class="settings">
      <div><dt>Water depth exaggeration</dt><dd>Deepens surveyed and predicted floors relative to their waterlines. At 4×, a 20 m depth is displayed as 80 m. This improves visibility; it does not improve the source data.</dd></div>
      <div><dt>Maximum depth</dt><dd>Changes the depth constraint for a modeled lake, including modeled survey gaps. It does not replace measured survey samples or turn a prediction into a survey.</dd></div>
      <div><dt>Material and sheet limits</dt><dd>A shallow lake may fall between contour levels and show no underwater step. Mountains can make each interval large. We reserve room for water within the 24-sheet limit, but the result still has a finite vertical resolution.</dd></div>
      <div><dt>Fit depth</dt><dd>Compresses an over-deep lake to fit the available stack, keeping its waterline fixed and scaling depths uniformly. If space is insufficient without fitting, the deepest floor can be flattened. “Use manual depth” restores your requested scale.</dd></div>
    </dl>
    <p><strong>If a lake looks flat:</strong> confirm that Water depth is enabled in layered mode, check the warnings, and try raising water depth exaggeration. In our full-lake Jenny Lake test, 1× showed no underwater contour while 4× showed two. Results depend on the crop and material settings. Flat engraving does not generate the stacked lake-depth recess.</p>
  </section>
  ...
  <section id="confidence" aria-labelledby="confidence-title">
    ...
    <div class="reading-key">
      ...
      <p><strong>Mixed:</strong> available survey samples are retained, with gaps filled from terrain or modeled depths where possible.</p>
    </div>
    <p>The studio displays <strong>“Some lake depths are estimated rather than surveyed”</strong> when a generated lake includes estimates. Exported project metadata keeps depth provenance, applied depth fitting, and warnings. Source credits accompany the export.</p>
