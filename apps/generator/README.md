...
`src/mcp-app/` is not part of the site. It is the in-chat preview that the map-api Worker serves as an MCP App (`ui://topostack/terrain-preview.html`): `vite.mcp-app.config.ts` builds it after the site into one self-contained `dist/mcp-app/terrain-preview.html`, which the Worker reads through its `ASSETS` binding. It speaks the MCP Apps postMessage protocol to the chat host (`host.ts`), loads terrain with `lib/domain/data-provider.ts` after pointing it at the Worker with `configureApiBase` (`lib/domain/api-base.ts`), generates with core, and draws with `render.ts`. It has its own bundle budget, is kept out of the Atomm package, and is never linked from the site. See [docs/mcp.md](../../docs/mcp.md#the-in-chat-preview-mcp-app).

The studio also offers its own tools to browser agents through WebMCP (`document.modelContext`, Chrome behind a flag or origin trial): `lib/studio/webmcp-tools.ts` defines them against a small host interface that `App.svelte` fills with its own update, generate, undo and export actions, so an agent's edit is an ordinary undo step. `App.svelte` loads `lib/studio/webmcp.ts` only when the browser has a model context and never in the Atomm embed, and the budget check fails if it reaches the startup path. Studio links with `?generate=1` (the links agents make) generate the shared design on arrival. See [docs/mcp.md](../../docs/mcp.md#webmcp-in-the-studio); the public guides are `src/routes/guides/{use-with-ai-assistants,browser-agents,mcp-server,agent-api}/`.
