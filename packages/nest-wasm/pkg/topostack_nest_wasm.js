 ...
 * @param {Function} on_report
 * @returns {string}
 */
export function stripPack(job_json, on_report) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(job_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.stripPack(ptr0, len0, on_report);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('topostack_nest_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
