...
async function fixture(t) {
  ...
  for (const path of ["apps/generator", "packages/chart-trace", "packages/core", "packages/data-contracts", "packages/nest-wasm", "workers/map-api", "atomm"]) await mkdir(new URL(path, base), { recursive: true });
  for (const path of ["package.json", "package-lock.json", "apps/generator/package.json", "packages/chart-trace/package.json", "packages/core/package.json", "packages/data-contracts/package.json", "packages/nest-wasm/package.json", "workers/map-api/package.json", "atomm/version.json"]) {
...
test("main bumps synchronize every workspace, lock entry and the Atomm manifest without changing dependency data", async (t) => {
  ...
  for (const path of ["", "apps/generator", "packages/chart-trace", "packages/core", "packages/data-contracts", "packages/nest-wasm", "workers/map-api"]) oldLock.packages[path].version = version;
